...
var Main = function() {
	...
	var sidebarHandler = function() {
		var eventObject = isTouch() ? 'click' : 'mouseenter', elem = $('#sidebar'), ul = "", menuTitle, _this, sidebarMobiler = $('.sidebar-mobile-r'), $winOffsetTop = 0, $winScrollTop = 0, $appWidth;
		...
		elem.on('click', 'a', function(e) {
			...
			if (_this.next().is('ul') && _this.parent().Class('open')) {

				_this.next().slide(200, function() {
					$win.trigger("resize");
		...
		sidebarMobiler.on('click', function() {
		...
		$(document).on("mousedown touchstart", function(e) {
			if (elem.has(e.target).length === 0 && !elem.is(e.target) && !sidebarMobiler.is(e.target) && ($('#app').hasClass('app-slide-off') || $('#app').hasClass('app-offsidebar-open'))) {
				resetSidebar();
			}
		});
	...
	var navbarHandler = function() {
		...
		$(document).on("mousedown touchstart", Navbar);
		function Navbar(e) {
	...
	// class
	var ClassOnElement = function() {
		var Attribute = $('*[data--class]');
		Attribute.each(function() {
			var _this = $(this);
			var Class = _this.attr('data--class');
			var outsideElement;
			var Element;
			typeof _this.attr('data--target') !== 'undefined' ? Element = $(_this.attr('data--target')) : Element = _this;
			_this.on("click", function(e) {
				if (_this.attr('data--type') !== 'undefined' && _this.attr('data--type') == "on") {
					Element.addClass(Class);
				} else if (_this.attr('data--type') !== 'undefined' && _this.attr('data--type') == "off") {
					Element.removeClass(Class);
				} else {
					Element.Class(Class);
				}
				e.preventDefault();
				if (_this.attr('data--click-outside')) {

					outsideElement = $(_this.attr('data--click-outside'));
					$(document).on("mousedown touchstart", Outside);
			...
			var Outside = function(e) {
				if (outsideElement.has(e.target).length === 0 && !outsideElement.is(e.target) && !Attribute.is(e.target) && Element.hasClass(Class)) {
					Element.removeClass(Class);
					$(document).off("mousedown touchstart", Outside);
				}
			};
	...
	var panelToolsHandler = function() {
		...
		$('body').on('click', '.panel-collapse', function(e) {
			...
			bodyPanel.slide(200, function() {
				panel.Class("collapses");
			});
